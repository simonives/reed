...
describe('data store', () => {
  ...
  it('exportBackup calls fetch with correct URL and API key header', async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith('/api/v1/data/export', {
      headers: { 'X-API-Key': 'test-key' },
    })
  })

  it('restoreBackup posts FormData to /data/restore and returns summary', async () => {
    ...
    expect(api.post).toHaveBeenCalledWith('/data/restore', expect.any(FormData))
