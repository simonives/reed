...
**PR #256 (2026-08-16)** fixed issue #185 — `PATCH /api/v1/share/targets/{id}` with an explicit `{"config": null}` against a seeded `copy_link`/`copy_markdown` target fell through `_validate_config`'s falsy check and persisted the literal string `"null"`, permanently crashing every subsequent `GET /share/targets` with no UI recovery path. Two-layer fix: the update endpoint now rejects an explicit null config with a 422, and `_redact_target` plus both graph-layer readers treat a null/missing config as `{}` defensively so an already-poisoned row self-heals on next read. Both layers independently mutation-tested; full suite 617 passed, 2 skipped.

Active session plan: two-tier punch list, agreed with Simon 2026-08-15 — **RC1 bar: cleared and validated** (#241 fixed and merged; Simon rebuilt the local RC1 image off current `main` and confirmed live against his real ~4,750-item library — CPU/memory settled to idle well within minutes, no repeat of the old 15-minute/6.6GB crisis). **v1.0.0 public bar:** M7 sub-projects 1 (distribution/SBOM) and 2 (documentation) now complete; sub-projects 3 (website) and 4 (public repository launch) remain, plus resolving #242 (auth model for public cloud hosting). Next up per Simon's direction (2026-08-16): a full open-issue backlog review, to catch anything high-priority or blocking before more net-new build work — backlog is 30 open issues as of 2026-08-22 (verified via `gh issue list`; the prior 2026-08-16 note put it at 113, so the gap between these two counts hasn't been reconciled and shouldn't be assumed to reflect only #185's closure). See Expiry for the refresh trigger on this subsection.
