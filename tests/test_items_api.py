...
class TestCursorPagination:
    ...
    def test_cursor_handles_null_published_at(self, authed, reed_client):
        ...
        page2 = reed_client.get(
            "/api/v1/items", params={"limit": 2, "cursor": cursor}, headers=authed.headers
        ).json()
    ...
    def test_cursor_pagination_starred_view(self, authed, reed_client):
        ...
        for i in range(4):
            ...
            reed_client.patch(
                f"/api/v1/items/{item_id}", json={"starred": True}, headers=authed.headers
            )
        page1 = reed_client.get(
            "/api/v1/items", params={"starred": "true", "limit": 2}, headers=authed.headers
        )
        ...
        page2 = reed_client.get(
            "/api/v1/items",
            params={"starred": "true", "limit": 2, "cursor": cursor},
    ...
    def test_cursor_pagination_tag_unread_view(self, authed, reed_client):
        ...
        page1 = reed_client.get(
            "/api/v1/items",
            params={"tag": "cursor-test-tag", "unread": "true", "limit": 2},
            headers=authed.headers,
        )
        assert page1.status_code == 200, page1.text
        cursor = page1.json()["meta"]["next_cursor"]
        ...
        page2 = reed_client.get(
            "/api/v1/items",
            params={"tag": "cursor-test-tag", "unread": "true", "limit": 2, "cursor": cursor},
            headers=authed.headers,
        )
        assert page2.status_code == 200, page2.text
